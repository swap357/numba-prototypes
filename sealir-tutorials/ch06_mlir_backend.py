...
class Backend:
    ...
    def lower(self, root: rg.Func, argtypes):
        ...
        self.i32 = i32 = ir.IntegerType.get_signless(32, context=context)
        self.i64 = i64 = ir.IntegerType.get_signless(64, context=context)
        self.boo = boo = ir.IntegerType.get_signless(1, context=context)
        ...
        def lower_expr(expr: SExpr, state: ase.TraverseState):
            match expr:
                ...
                case NbOp_Div_Int64(lhs, rhs):
                    ...
                    return arith.sitofp(self.f64, arith.divsi(lhs, rhs))
                ##### more
                ...
                case rg.Loop(body=rg.RegionEnd() as body, operands=operands):
                    ...
                    # Before Region
                    with ir.InsertionPoint(before_block), push(new_ops):
                        values = yield body
